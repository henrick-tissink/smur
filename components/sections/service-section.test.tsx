...
describe("ServiceSection", () => {
  it("renders the service section with its id and heading + body", () => {
    const { container } = renderWithIntl(<ServiceSection service={services[0]} />);
  ...
  it("renders the DETAILS + TIMELINE accordions", () => {
    renderWithIntl(<ServiceSection service={services[0]} />);
    expect(screen.getByRole("button", { name: /details/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /timeline/i })).toBeInTheDocument();
  });
  it("data-nav-scheme is dark", () => {
    const { container } = renderWithIntl(<ServiceSection service={services[1]} />);
